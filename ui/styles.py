...
# ═══ PATCH AUTH v5 — inyectado al final para mayor especificidad ═══
def inject_auth_patch():
    """CSS adicional de alta especificidad para la pantalla de auth."""
    import streamlit as st
    from ui.styles import get_theme
    dark = get_theme() == "dark"
    input_bg  = "#080d14" if dark else "#f4f8fc"
    input_bdr = "#1e3048" if dark else "#d0dcea"
    input_txt = "#c8d8ea" if dark else "#1a2b3c"
    ...
    st.markdown(f"""
<style>
/* Ocultar el botón real de Streamlit (usamos solo el HTML fijo) */
[data-testid="stVerticalBlock"] > div:first-child .stButton {{
...
[data-testid="stVerticalBlock"] > div:first-child .stButton > button {{
    ...
    background: {'rgba(12,18,25,0.80)' if dark else 'rgba(255,255,255,0.90)'} !important;
    backdrop-filter: blur(12px) !important;
    color: {'#c8d8ea' if dark else '#2a3f54'} !important;
    -webkit-text-fill-color: {'#c8d8ea' if dark else '#2a3f54'} !important;
    border: 1px solid {'rgba(200,216,234,0.15)' if dark else 'rgba(0,0,0,0.10)'} !important;
...
/* Inputs: borde en las CUATRO esquinas — fix border-radius */
.stTextInput > div > div,
.stTextInput [data-baseweb="input"] {{
    border-radius: 10px !important;
    overflow: hidden !important;
}}
/* Asegurar que ninguna capa interna tenga fondo propio */
.stTextInput > div > div > div,
.stTextInput [data-baseweb="base-input"] > div {{
    background: transparent !important;
    border: none !important; box-shadow: none !important;
}}
/* Botón ojo completamente sin fondo */
.stTextInput button,
[data-testid="stTextInputRootElement"] button,
.stTextInput div[data-testid] button {{
    background: transparent !important;
